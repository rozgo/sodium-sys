...
/// use sodium_sys::crypto::symmetrickey::{aead,key};
///
...
#[cfg(feature = "latest")]
/// The *aead_encrypt_ietf()* function encrypts a message using a secret key and
/// a public nonce.
///
/// The encrypted message, as well as a tag authenticating both the confidential
/// message and non-confidential data, are put into the output byte sequence.
///
/// Associated data is optional and can be None if no associated data are
/// required..  Also, the associated data should be less than ABYTES in length.
///
/// The public nonce should never ever be reused with the same key. The
/// recommended way to generate it is to use the Nonce struct for the first
