...
/// The *random_byte_array()* function fills the given mutable byte array with an unpredictable
/// sequence of bytes.
///
...
pub fn random_byte_array(buf: &mut [u8]) {
    unsafe {
        randombytes_buf(buf.as_mut_ptr() as *mut ::libc::c_void, buf.len() as ::libc::size_t);
    }
}
